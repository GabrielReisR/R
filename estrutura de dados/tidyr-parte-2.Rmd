...
biblios_replace <- biblios_exp %>% 
  replace_na(list(total_librarians = floor(mean(biblios_exp$total_librarians, na.rm = T)),
                  total_users = floor(mean(biblios_exp$total_users, na.rm = T))
  ))
...
Olhemos para o resultado agora. Basta olhar para *total_librarians* para vermos que a média talvez não seja a melhor representação de tendência central dessa variável. O valor novo para o id 82 é **1587**. E, para *total_users* no id 77, é de **1229871**. Vimos aqui o problema de se substituir pela média. Ela é facilmente influenciada por valores extremos.

# Imputação por regressão

Podemos personalizar ainda mais, caso a caso, o valor das três variáveis iniciais que tinham missings: *total_librarians*, *total_users* e *expenditures_us_dollars*.

Vamos carregar o banco inicial novamente.

```{r Carregando o banco inicial novamente}
missmap(biblios) # vendo novamente os missings
head(biblios)
```
